...
// gemv computes y[j] = bias[j] + sum_d x[d] * W[d*cols + j] for j in [0,
// cols). W is [rows, cols] row-major; len(x) == rows; len(y) >= cols. If
// bias is nil the bias term is zero.
//
// Block-j-outer: each 16-wide output block stays in a single Float32x16
// register across all `rows` input reductions, so the whole reduction
// costs ONE store per block of 16, vs one store per (d, block) in the
// axpy formulation. On the MoE gate-up matmul this cuts store traffic by
// D=640×.
func gemv(x, W, bias, y []float32, rows, cols int) {
	...
	for ; j+16 <= cols; j += 16 {
		...
		if bias != nil {
			acc = archsimd.LoadFloat32x16Slice(bias[j:])
		} else {
			acc = archsimd.BroadcastFloat32x16(0)
		}
		for d := 0; d < rows; d++ {
			s := archsimd.BroadcastFloat32x16(x[d])
			w := archsimd.LoadFloat32x16Slice(W[d*cols+j:])
			acc = s.MulAdd(w, acc)
		}
		acc.StoreSlice(y[j:])
	}
	if j+8 <= cols {
		...
		if bias != nil {
			acc = archsimd.LoadFloat32x8Slice(bias[j:])
		} else {
			acc = archsimd.BroadcastFloat32x8(0)
		}
		for d := 0; d < rows; d++ {
			s := archsimd.BroadcastFloat32x8(x[d])
			w := archsimd.LoadFloat32x8Slice(W[d*cols+j:])
			acc = s.MulAdd(w, acc)
		}
		acc.StoreSlice(y[j:])
		j += 8
	}
	// Scalar tail for any residual cols (uncommon on the current model).
	for ; j < cols; j++ {
		var s float32
		if bias != nil {
			s = bias[j]
