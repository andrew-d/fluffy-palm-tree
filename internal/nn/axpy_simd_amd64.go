...
// addInPlace computes a[i] += b[i] in-place. 16-wide SIMD with a 2-way
// outer unroll so two independent add chains are in flight per iter.
// Used by the transformer's residual additions.
func addInPlace(a, b []float32) {
	...
	for ; i+32 <= n; i += 32 {
		...
		av0.Add(bv0).StoreSlice(a[i:])
		av1.Add(bv1).StoreSlice(a[i+16:])
	}
	for ; i+16 <= n; i += 16 {
		av := archsimd.LoadFloat32x16Slice(a[i:])
		bv := archsimd.LoadFloat32x16Slice(b[i:])
		av.Add(bv).StoreSlice(a[i:])
	}
	for ; i < n; i++ {
		a[i] += b[i]
	}
