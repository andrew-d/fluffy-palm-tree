...
// axpyBatch16Seed is axpyBatch16 but for the FIRST d-chunk: instead of
// loading y[k][i:+16] (which hasn't been written yet), it seeds from a
// shared `bias[i:+16]` vector. Removes the explicit bias-copy pass the
// MoE path used to do before its axpyBatch cascade.
func axpyBatch16Seed(
	a [16][]float32,
	w [16][]float32,
	bias []float32,
	y []float32, stride int,
	...
	for ; i+16 <= width; i += 16 {
		...
		wv14 := archsimd.LoadFloat32x16Slice(w[14][i:])
		wv15 := archsimd.LoadFloat32x16Slice(w[15][i:])
		biasV := archsimd.LoadFloat32x16Slice(bias[i:])
		for k := 0; k < n; k++ {
			off := k*stride + i
			yv := biasV
			yv = archsimd.BroadcastFloat32x16(a[0][k]).MulAdd(wv0, yv)
