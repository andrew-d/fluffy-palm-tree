...
// moeGemmTile4x4 computes a 4 (token) × 64 (output-lane) BLIS-style C-tile
// of the MoE GEMM C[M, N] = A[M, K] * B[K, N] + bias, holding 16
// Float32x16 accumulators across the whole K reduction. Each accumulator
// IS the 16-wide SIMD vector representing 16 consecutive output columns
// of one token (no hsum needed — unlike linearTile4x4 which does dot
// products). Per K step: 4 B-loads + 4 A-broadcasts + 16 FMAs, for an
// arithmetic intensity of 2 FMAs per load.
//
//	a0..a3: base offsets into A for the 4 participating tokens (A is the
//	        hidden-state array indexed by pair.t * D).
//	kStride: stride between K steps in A (= D for hidden, = I for gated).
//	bBase, bStride: base and K-stride into B (expert's weight panel).
//	j0: output column offset where this tile starts.
//	nCols: length of each B row / bias row (= twoI for gate-up, = D for down).
//	K: reduction length (= D for gate-up, = I for down).
//	cBase, cStride: where to store the tile's MR rows in C.
func moeGemmTile4x4(
	...
	// Seed the 16 accumulators from bias (same bias row for every token).
	bv0 := archsimd.LoadFloat32x16Slice(bias[j0:])
	bv1 := archsimd.LoadFloat32x16Slice(bias[j0+16:])
	bv2 := archsimd.LoadFloat32x16Slice(bias[j0+32:])
	bv3 := archsimd.LoadFloat32x16Slice(bias[j0+48:])
	c00, c01, c02, c03 := bv0, bv1, bv2, bv3
	c10, c11, c12, c13 := bv0, bv1, bv2, bv3
	c20, c21, c22, c23 := bv0, bv1, bv2, bv3
	c30, c31, c32, c33 := bv0, bv1, bv2, bv3

	for k := 0; k < K; k++ {
		rowBase := bBase + k*bStride + j0
		wv0 := archsimd.LoadFloat32x16Slice(B[rowBase:])
		wv1 := archsimd.LoadFloat32x16Slice(B[rowBase+16:])
		wv2 := archsimd.LoadFloat32x16Slice(B[rowBase+32:])
		wv3 := archsimd.LoadFloat32x16Slice(B[rowBase+48:])
		...
		c02 = s0.MulAdd(wv2, c02)
		c03 = s0.MulAdd(wv3, c03)
		c10 = s1.MulAdd(wv0, c10)
		c11 = s1.MulAdd(wv1, c11)
		c12 = s1.MulAdd(wv2, c12)
		c13 = s1.MulAdd(wv3, c13)
		c20 = s2.MulAdd(wv0, c20)
		c21 = s2.MulAdd(wv1, c21)
		c22 = s2.MulAdd(wv2, c22)
		c23 = s2.MulAdd(wv3, c23)
		c30 = s3.MulAdd(wv0, c30)
		c31 = s3.MulAdd(wv1, c31)
		c32 = s3.MulAdd(wv2, c32)
		c33 = s3.MulAdd(wv3, c33)
	}
	...
	c02.StoreSlice(C[o0+32:])
	c03.StoreSlice(C[o0+48:])
	c10.StoreSlice(C[o1:])
	c11.StoreSlice(C[o1+16:])
	c12.StoreSlice(C[o1+32:])
	c13.StoreSlice(C[o1+48:])
	c20.StoreSlice(C[o2:])
	c21.StoreSlice(C[o2+16:])
	c22.StoreSlice(C[o2+32:])
	c23.StoreSlice(C[o2+48:])
	c30.StoreSlice(C[o3:])
	c31.StoreSlice(C[o3+16:])
	c32.StoreSlice(C[o3+32:])
	c33.StoreSlice(C[o3+48:])
}
